/**
 * collection-quick-add.js - uses QuantityUtils for quantity behaviour and handles Ajax add-to-cart
 */
(function(utils){
  var clearSelection = utils ? utils.clearTextSelection : function(){};

  function attachNoHighlightListeners(){
    document.addEventListener('click', function(e){
      var btn = e.target.closest('.collection-add-to-cart, .double-qty-btn, .collection-qty-button, .sf__btn');
      ...
      fetch(`${settings.routes?.cart_add_url || '/cart/add'}`, config)
        ...
        .then(() => {
          if(resetQty && qtyInput){
            ...
            if(utils){
              utils.validateAndHighlightQty(qtyInput);
              utils.updateQtyButtonsState(qtyInput);
            }
          }
